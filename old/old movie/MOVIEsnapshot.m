function MOVIEsnapshot(str_input,apstr)
close all;
...
%     [pot,target]=VField(i);
%     quiver(target(2,:),-target(1,:),imag(pot),-real(pot));hold on;
    hold off;
...
% msize=.01;
% for k=1:pos
%     p1=plot(imag(Wing(ib).ZetaF{tk}(piece_p(k,1):piece_p(k,2))),-real(Wing(ib).ZetaF{tk}(piece_p(k,1):piece_p(k,2))),...
%         'r.','MarkerSize',msize);hold on;
% %     set(p1,'color',[.5 .5 .5]);
% end
% for k=1:neg
%     p2=plot(imag(Wing(ib).ZetaF{tk}(piece_n(k,1):piece_n(k,2))),-real(Wing(ib).ZetaF{tk}(piece_n(k,1):piece_n(k,2))),...
